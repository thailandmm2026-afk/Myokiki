...
# ---------------- CONFIG ----------------
# Prefer env BOT_TOKEN (Render). Fallback only for local test.
BOT_TOKEN = os.getenv("BOT_TOKEN", "").strip() or "8457704294:AAFmv7Z7DQZbXENRNNsaUu0dMiF7Bx4uvIw"
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "").strip() or "AQ.Ab8RN6Jghec9VDkQFrSWbff3DZbGW3gJK11lBIsMYfKkLbK6CQ"
...
def db():
    parent = os.path.dirname(os.path.abspath(DB_PATH or "kiki_shop.db"))
    if parent and not os.path.isdir(parent):
        try:
            os.makedirs(parent, exist_ok=True)
        except OSError:
            log.warning("Cannot create DB directory: %s", parent)
...
def build_application():
    """Build PTB Application with all handlers (shared by polling / webhook)."""
    if not BOT_TOKEN:
        raise RuntimeError(
    ...
    application.add_handler(CommandHandler("setwebpin", setwebpin_cmd))
    application.add_handler(CommandHandler("claimmission", claimmission_cmd))
    for command in (
        ...
        application.add_handler(CommandHandler(command, admin_cmd))
...
def main():
    """Polling mode (local / VPS). On Render use app.py webhook instead."""
    application = build_application()
    log.info(
        "LEO SHOP v18.7 polling | Gemini=%s | model=%s | Tesseract=%s",
