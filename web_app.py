...
BOT_TOKEN = os.getenv("BOT_TOKEN", "").strip() or "8457704294:AAFmv7Z7DQZbXENRNNsaUu0dMiF7Bx4uvIw"
OWNER_ID = int(os.getenv("OWNER_ID", "7308292609"))
ADMIN_IDS = {OWNER_ID}
KPAY_NUMBER = os.getenv("KPAY_NUMBER", "09446787195")
KPAY_NAME = os.getenv("KPAY_NAME", "Mg Myo Min")
WAVE_NUMBER = os.getenv("WAVE_NUMBER", "09667102005")
WAVE_NAME = os.getenv("WAVE_NAME", "Mg Myo Min")
...
# ---------------- DB ----------------
def _ensure_db_dir():
    """Create parent folder for DB / cookie paths (Render /var/data, etc.)."""
    for path in (DB_PATH, COOKIE_FILE, SMILE_PRICES_FILE):
        parent = os.path.dirname(os.path.abspath(path or ""))
        if parent and not os.path.isdir(parent):
            try:
                os.makedirs(parent, exist_ok=True)
            except OSError as e:
                log.warning("Cannot create dir %s: %s", parent, e)
...
# ---------------- TELEGRAM NOTIFY ----------------
def notify_telegram_admins(text: str, reply_markup: dict = None) -> int:
    """Send HTML message (+ optional inline keyboard) to all admins."""
...
def api_register():
    """Create web account: Telegram User ID + PIN (no bot /start required)."""
    ...
    if user_id <= 0:
        return jsonify({"ok": False, "error": "User ID မမှန်ပါ။"}), 400
    if len(pin) < 4:
        ...
        if stored:
            return (
                jsonify(
                    {
                        "ok": False,
                        "error": "ဒီ ID နဲ့ အကောင့်ရှိပြီးပါပြီ။ Login လုပ်ပါ။",
                    }
                ),
                409,
            )
        # Exists from bot but no PIN yet → set PIN
        set_web_pin(user_id, pin)
        if name:
            with db() as conn:
                conn.execute(
                    "UPDATE users SET first_name=? WHERE user_id=? AND (first_name='' OR first_name IS NULL)",
                    (name, user_id),
                )
    else:
        ensure_user(user_id, first_name=name or f"User{user_id}")
        set_web_pin(user_id, pin)
    ...
    if not row:
        return (
            jsonify(
                {
                    "ok": False,
                    "error": "အကောင့်မရှိပါ။ အောက်က 「အကောင့်ဖွင့်မည်」 ကိုနှိပ်ပါ။",
                    "need_register": True,
                }
            ),
            404,
...
def api_recharge():
    ...
    notify_text = (
        ...
        f"အောက်က ခလုတ်ဖြင့် Approve / Reject လုပ်ပါ။"
    )
    # Inline buttons (need bot webhook/polling running to receive callbacks)
    kb = {
        "inline_keyboard": [
            [
                {
                    "text": "✅ APPROVE",
                    "callback_data": f"admin|approve|recharge|{rid}",
                },
                {
                    "text": "❌ REJECT",
                    "callback_data": f"admin|reject|recharge|{rid}",
                },
            ]
        ]
    }
    sent = notify_telegram_admins(notify_text, reply_markup=kb)
